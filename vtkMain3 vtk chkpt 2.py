...
class VTKEngine:
    ...
    def _apply_transform(self):
        ...
        center = np.array(img.GetCenter())  # center in world coordinates
...
class FusionUI(QtWidgets.QWidget):
    ...
    def _build(self):
        ...
        root.addWidget(left, 1)   # left toolbar width
        root.addWidget(right, 2)  # slice viewers
...
class Controller(QtCore.QObject):
    ...
    def __init__(self, ui: FusionUI, engine: VTKEngine):
        ...
        self._debounce_timers = {
            "tx": QtCore.QTimer(singleShot=True),
            "ty": QtCore.QTimer(singleShot=True),
            "tz": QtCore.QTimer(singleShot=True),
            "rx": QtCore.QTimer(singleShot=True),
            "ry": QtCore.QTimer(singleShot=True),
            "rz": QtCore.QTimer(singleShot=True),
            "opacity": QtCore.QTimer(singleShot=True),
        }
    ...
    def _wire(self):
        ...
        # Wrap slider signals with debounce
        self.ui.txChanged.connect(lambda v: self._debounced_set("tx", lambda: self.engine.set_translation(v, self.engine._ty, self.engine._tz)))
        self.ui.tyChanged.connect(lambda v: self._debounced_set("ty", lambda: self.engine.set_translation(self.engine._tx, v, self.engine._tz)))
        self.ui.tzChanged.connect(lambda v: self._debounced_set("tz", lambda: self.engine.set_translation(self.engine._tx, self.engine._ty, v)))

        self.ui.rxChanged.connect(lambda v: self._debounced_set("rx", lambda: self.engine.set_rotation_deg(v, self.engine._ry, self.engine._rz)))
        self.ui.ryChanged.connect(lambda v: self._debounced_set("ry", lambda: self.engine.set_rotation_deg(self.engine._rx, v, self.engine._rz)))
        self.ui.rzChanged.connect(lambda v: self._debounced_set("rz", lambda: self.engine.set_rotation_deg(self.engine._rx, self.engine._ry, v)))

        self.ui.opacityChanged.connect(lambda a: self._debounced_set("opacity", lambda: self.engine.set_opacity(a)))
    ...
    def _debounced_set(self, name: str, setter_callable):
        """Run setter and start debounce timer to refresh_all"""
        setter_callable()
